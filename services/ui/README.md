...
`web/` はNext.js + TypeScriptで作った、FastAPI検索API用の画面です。

### 起動

1. `web/.env.example` を `web/.env.local` にコピーし、`NEXT_PUBLIC_SEARCH_API_URL` を検索APIのURLにします。Supabaseを使う場合はURLとanon keyだけを設定します。Service Role KeyやGeminiキーはブラウザに置きません。
2. `cd web` → `npm install` → `npm run dev`
3. ブラウザで `http://localhost:3000` を開きます。

送信時、画面は検索APIの `POST /search` に `{ "query": "質問文" }` を送ります。レスポンスは `{ "answer": "回答", "sources": [{ "file_name": "資料.pdf", "page": 3 }] }` を想定しています。APIの実装に合わせて項目名を調整してください。

### UIの確認

APIキーなしで `npm run typecheck` または `npm run build` を実行できます。検索APIが起動していない場合は、画面にエラーを表示します。
